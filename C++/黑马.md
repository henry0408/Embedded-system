...
默认调用的是子类的成员，如果想访问父类成员，需要加一个作用域::
...
由于静态多态，函数地址早绑定，所以不会输出cat is speaking
